...
def acao_botao(canva, frame):
    dicionario = abre_boletim()
    contador = 0
    round_rectangle(0, 0, 528, len(dicionario) * 45, canva, radius=31, fill="#242A33")
    for i in dicionario.keys():
        canva.create_image(211.0, 32 + 44 * contador, image=bloco)
        canva.create_text(
            60.0,
            20 + contador * 44.0,
            anchor="nw",
    ...
    if contador > 5:
        ...
        canva.config(yscrollcommand=vbar.set)
        canva.pack(side=LEFT, expand=True, fill=BOTH)
        canva.config(scrollregion=(-1, -1, 529, contador * 45))
...
button_1 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: acao_botao(Canvas_notas, central),
    relief="flat",
)
button_1.place(x=4.0, y=479.0, width=110.0, height=30.0)

central = Frame(window, width=528, height=351)
